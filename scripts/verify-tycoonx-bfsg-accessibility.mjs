...
requireMatch(gate, /Marktüberwachungsstelle der Länder für die Barrierefreiheit von Produkten und Dienstleistungen \(MLBF\)/i, 'Missing current MLBF authority checkpoint.');
...
// Nonconformity and narrow exceptions.
requireMatch(gate, /Nonconformity, correction, and authority notification/i, 'Missing BFSG nonconformity workflow.');
...
if (errors.length) {
  ...
  console.log('PASS: BFSG scope/exemption, Annex 3, e-commerce accessibility, provider boundaries, and TycoonX paid-entitlement safeguards are present.');
}
